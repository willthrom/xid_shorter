...
var IDs = []IDParts{
	{
		ID{0x4d, 0x88, 0xe1, 0x5b, 0x60, 0xf4, 0x86, 0xe4},
		1300816219,
		[]byte{0x60, 0xf4, 0x86},
		0x28,
		1561,
	...
	{
		ID{0x00, 0x00, 0x00, 0x00, 0xaa, 0xbb, 0xcc, 0xdd},
		0,
		[]byte{0xaa, 0xbb, 0xcc},
		0xee,
		1,
	...
	for i := 1; i < len(ids); i++ {
		...
		for j, tid := range ids {
			if j != i {
				if id.Compare(tid) == 0 {
					id := ids[i]
					t.Errorf("generated ID is not unique (%d/%d): %d", i, j, id.UInt64())
				}
			}
		}
...
func TestIDString(t *testing.T) {
	id := ID{0x4d, 0x88, 0xe1, 0x5b, 0x60, 0xf4, 0x86, 0xe4}
...
func TestFromString(t *testing.T) {
	...
	want := ID{0x4d, 0x88, 0xe1, 0x5b, 0x60, 0xf4, 0x86, 0xe4}
	if got != want {
...
func TestID_Compare(t *testing.T) {
	pairs := []struct {
		...
		{IDs[1].id, IDs[0].id, -1},
		{ID{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, IDs[2].id, -1},
		{IDs[0].id, IDs[0].id, 0},
	}
	for _, p := range pairs {
